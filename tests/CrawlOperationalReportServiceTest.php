...
final class CrawlOperationalReportServiceTest extends TestCase
{
    ...
    public function testAggregatesJobsAndUrlsAndSupportsDomainAndLimit(): void
    {
        ...
        $pdo->exec("INSERT INTO crawl_urls (domain,url,status,http_status,error_message,updated_at) VALUES
            ('example.com','https://example.com/1','queued',NULL,NULL,'2026-05-08 10:00:00'),
            ('example.com','https://example.com/2','failed',503,'timeout','2026-05-08 11:00:00'),
            ('other.com','https://other.com/1','indexed',200,NULL,'2026-05-08 09:00:00')");
    ...
    public function testCommandRegistrationAndReadOnlyContractChecks(): void
    {
        ...
        preg_match('/private function crawlReport\(array \$argv\): int\s*\{(?P<body>.*?)\n    \}\n\n    \/\*\* @return array<int,string> \*\//s', $kernel, $m);
        $body = (string) ($m['body'] ?? '');
    ...
    private function buildPdo(): PDO
    {
        ...
        $pdo->exec('CREATE TABLE crawl_urls (id INTEGER PRIMARY KEY AUTOINCREMENT, domain TEXT NOT NULL, url TEXT NOT NULL, status TEXT NOT NULL, http_status INTEGER NULL, error_message TEXT NULL, updated_at TEXT NULL)');
